...
export default function TallacUserCard({ user, isSelected = false, viewMode = 'card', onClick }: TallacUserCardProps) {
  const getStatusBorderClass = (status?: string) => {
    const statusMap: Record<string, string> = {
      Active: 'border-transparent hover:border-green-500',
      Inactive: 'border-transparent hover:border-red-500',
    };
    // Default: transparent border with blue hover (matching Vue3)
    return statusMap[status || ''] || 'border-transparent hover:border-blue-500';
  };
  ...
  if (viewMode === 'list') {
    return (
      <div
        onClick={onClick}
        className={`bg-gray-800 rounded-lg shadow-lg transition-all duration-200 hover:shadow-xl hover:bg-gray-750 border-2 cursor-pointer p-4 grid gap-4 items-center ${
      ...
      >
        ...
        <div className="min-w-0">
          {user.status ? (
            <span
              className={`inline-flex text-xs font-medium px-2.5 py-0.5 rounded-full whitespace-nowrap ${getStatusBadgeClass(
                user.status
              )}`}
            >
              {user.status}
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-gray-800 rounded-lg shadow-lg transition-all duration-200 hover:shadow-xl hover:bg-gray-750 border-2 cursor-pointer flex flex-col ${
        isSelected ? 'ring-2 ring-gray-700' : ''
      } ${getStatusBorderClass(user.status)}`}
    >
      <div className="p-4">
